...
class UserCacheImpl(private val config: RealmConfiguration) : IUserCache {

    private val realm: Realm get() = Realm.open(config)
    override suspend fun saveUser(userModel: UserModel) {
        val realm = realm
        realm.write {
            copyToRealm(userModel.let { UserDao(it.id, it.username, it.fullName, it.avatarUrl, it.publicEmail) })
        }
        realm.close()
    }
    ...
    override suspend fun getUser(): UserModel? {
        ...
        val user = result.singleOrNull()?.let { UserModel(it.id, it.username, it.name, it.avatarUrl, it.publicEmail) }
        realm.close()
        return user
    }
